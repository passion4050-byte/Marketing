-- ⚠️ 이 파일은 Round 191 로 대체됐다 → db/supabase/round191_cron_endpoints.sql
--
-- 아래 방식(cron.schedule 명령문 안에 URL+시크릿을 직접 박는 것)은 실행하지 말 것.
--   · public 리포라 정본을 커밋할 수 없어 "사용자가 손으로 치환" 숙제가 되었고,
--     그 상태로 한 달 가까이 방치돼 감시자가 한 번도 돌지 못했다.
--   · cron.job.command 에 시크릿이 평문으로 남는다.
-- Round 191 은 deploy_hooks 와 같은 패턴(값은 테이블 행, cron 은 함수만 호출)으로
-- 바꿔서 등록을 끝냈다. 지금 스케줄은 이미 걸려 있고, 남은 건 시크릿 UPDATE 한 줄이다.
-- 아래 내용은 설계 근거(왜 pg_cron 인가·스케줄 근거) 기록용으로만 남긴다.
--
-- ────────────────────────────────────────────────────────────────
