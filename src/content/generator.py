...
# 🔴 Round 181 (2026-08-30) — 네이버 AI 브리핑(AEO) 공식 가이드 반영.
#   출처: NAVER Search & Tech 「AI 시대에 사용자의 선택을 받는 콘텐츠 작성 가이드」
#         https://blog.naver.com/naver_search/224296857688 (2026-05-26)
#
#   왜 이걸 넣는가 — 서로 독립적인 실측 3건이 같은 결론을 가리킨다:
#     ① 우리 GSC:    1편 3위 → AI 인용 4회 / 369편 17위 → 0회
#     ② 우리 네이버: 질문형 롱테일 '백옥주사 효과는 얼마나 지속되나요?' CTR 14.3%, 클릭 1위
#     ③ 네이버 공식 FAQ 8: "검색 결과 상위에 노출되는 글이 AI 브리핑에 잘 인용되나요?
#        → 네 그렇습니다."
#   즉 구글이든 네이버든 **인용은 랭킹의 함수**다. 같은 규칙이 두 엔진에 다 통한다.
#
#   ⚠ 네이버가 명시적으로 "지양"이라고 쓴 것 중 우리가 실제로 저지를 수 있는 것 두 가지:
#     - "사람이 직접 쓴 문장이 아닌 AI가 기계적으로 생성·요약한 글을 무분별하게 발행"
#       → 우리는 자동 생성 파이프라인이다. 이 디렉티브가 그 방어선이다.
#     - "홍보만을 목적으로 외부로 이동하는 URL을 과도하게 삽입"
#       → 본문 외부 링크 남발 금지. (네이버 블로그에 백링크를 심는 전략도 여기에 걸린다.)
_NAVER_AEO_DIRECTIVE = (
    "[네이버 AI 브리핑 인용 기준 — 네이버 공식 가이드(2026-05) 반영]\n"
    "- **구체성이 진정성을 대신한다.** 일반론 대신 판단에 쓰이는 수치·범위·기간·조건을"
    " 문장 안에 직접 적으세요. '개인차가 있습니다'로 끝내지 말고 무엇이 개인차를 만드는지"
    "(각막 두께·도수·나이·기존 질환 등)를 이름으로 적습니다.\n"
    "- 통계·연구·공식 기관 자료를 인용할 때는 **출처 기관명과 연도**를 본문에 함께 적으세요."
    " 확인되지 않은 수치는 아예 쓰지 않습니다.\n"
    "- **이미지로만 전달되는 핵심 정보를 만들지 마세요.** 표·그림으로 보여준 내용은"
    " 반드시 텍스트로도 같은 정보를 남깁니다(AI 는 이미지 안을 읽지 못합니다).\n"
    "- 소제목은 실제 검색 질의형으로, 각 소제목 바로 아래 첫 문장이 그 질문의 **즉답**이어야"
    " 합니다. 그 한 문장만 떼어 인용돼도 말이 되어야 합니다.\n"
    "- 🔴 외부로 나가는 URL 을 홍보 목적으로 여러 개 넣지 마세요"
    " (네이버 가이드가 명시적으로 지양하는 항목). 근거 출처 링크는 예외입니다.\n"
    "- 🔴 맥락과 무관한 키워드를 노출 목적으로 끼워 넣지 마세요. 본문과 무관한 이미지도 금지.\n"
    "- 한 편의 글은 **하나의 주제**를 끝까지 다룹니다. 여러 시술을 얕게 나열하지 말고"
    " 이 글이 답하기로 한 질문에 깊이로 답하세요"
    " (네이버는 주제 일관성으로 '전문 출처'를 판별합니다)."
)
