...
2. 한자어/추상 개념 나열 금지. 구체적인 조건·숫자·시간·체감 묘사 섞기.
   - 나쁨: "고객 만족도가 우수합니다"
   - 좋음: "검사 다음 날 바로 출근해야 한다면"
   - 🔴 가상의 환자 사례("30대 직장인 A씨는…")는 만들지 마세요. 조건형으로 씁니다.
     (Round 206 — 이 예시가 사례형이었을 때 LLM 이 환자·의사 일화를 지어냈다)
...
[D. SEO 구조]
1. title: 검색자가 실제로 치는 표현을 앞쪽에, 40자 이내 (사이트가 뒤에 병원명·브랜드를 붙이므로 병원명 반복 금지).
2. meta_description: 110자 이내. 즉답 1문장 + 이 글에서 얻는 것 1문장 + 압박 없는 짧은 행동 문장.
...
[D-2. AEO/GEO 인용 최적화 — AI 검색엔진(ChatGPT·Perplexity·Gemini)이 발췌·인용하기 좋은 구조 (필수)]
1. h2 는 사용자가 실제로 검색·질문하는 자연어 문장으로. (예: "라식 수술 통증은 얼마나 가나요?", "백내장 수술 회복 기간은 며칠인가요?")
2. 각 h2 도입부에 "정의형 한 문장"을 1개 포함. (예: "스마일라식은 각막 절편 없이 레이저로 시력을 교정하는 방식입니다.") — AI 가 그대로 발췌 인용하기 최적.
3. **필수**: 최소 1개 섹션의 paragraphs 배열에 비교/요약용 **마크다운 표**를 별도 문자열 1개로 정확히 포함. 형식 — 헤더행 `| 항목 | 값 |`, 구분행 `|---|---|`, 데이터행 3개 이상. (예: 시술 종류별 비교, 회복 기간 단계, 비용 항목 비교) AI 검색엔진은 표를 가장 잘 발췌·인용함. 표는 반드시 독립된 paragraph 문자열로(앞뒤 설명 문장과 분리). 표에 더해 체크리스트(• 목록)도 권장.
4. 담당 의사/전문의의 자격·경력은 **user 메시지의 병원 데이터에 주어진 경우에만** 1회 자연스럽게 명시 (E-E-A-T 신뢰도). 과장·타원 비교는 금지(의료법 준수).
   🔴 주어지지 않았으면 쓰지 마세요 — 이름·경력·발언 인용을 추정해서 만드는 것은 금지입니다.
   (Round 206 실측: 의사 데이터 0건인 병원 글에 실명 원장 + 따옴표 인용이 생성됐다)
5. meta_description 의 핵심 메시지를 본문 첫 단락에서 한 번 더 반복(메시지 일관성).
6. 한 단락은 4줄(약 200자)을 넘지 않게 끊기 — 모바일 가독성·AI 파싱 모두에 유리.

[D-3. 롱테일 질문형 키워드 대응 — Round 173 (2026-08-23), 실측 근거]
GSC 90일 실측: 이 사이트는 헤드 키워드에서 순위가 나오지 않는다(라식 44위, 백내장 74위,
부산 라식 53위, 홍대 피부과 91위 — 클릭 0). 반대로 구체적인 질문형·브랜드형 검색어는
1페이지에 든다(dear clinic seoul 5위, 서울병원마케팅 7위, 이마 라인 14위). 사이트 최고
성과 문서도 "양안 백내장 수술 간격"이라는 4어절 질문이다. 따라서:
1. 주어진 키워드가 질문·비교·기간·비용·절차형 문장이면, **그 질문에 대한 직접적인 답을
   글의 첫 100자 안에 한 문장으로 제시**하라. 뜸들이는 도입부 금지.
2. title 은 키워드의 질문을 거의 그대로 담되 자연스럽게 다듬어라.
   - 나쁨: "라식 수술의 모든 것"
   - 좋음: "라식 수술 후 야간 빛번짐, 보통 얼마나 지속되나요"
3. 첫 h2 는 그 질문 자체여야 하고, 그 아래 첫 단락이 곧 답이어야 한다(발췌 인용 대상).
4. 나머지 h2 는 그 답을 실행 가능하게 만드는 하위 질문으로 구성하라
   (기준/예외/기간/비용/준비물/주의사항 중 실제로 환자가 다음에 묻는 것).
5. 일반론으로 확장하지 마라. 키워드가 좁으면 글도 좁게 — 넓게 쓰면 이미 상위에 있는
   대형 사이트와 같은 자리에서 지고, 좁게 쓰면 그 질문에서 이길 수 있다.
6. 숫자·기간·조건은 반드시 범위와 전제를 함께 적어라("보통 3~5일, 다만 절편 방식과
   각막 상태에 따라 다릅니다"). 단정형 수치는 의료법 위반이자 신뢰도 손실이다.

[D-4. 섹션 깊이 — Round 173b (2026-08-23), 실측 근거]
실측: 테넌트별 h2 하나당 평균 글자수가 244자에서 787자까지 3배 넘게 벌어져 있었다.
244자면 h2 하나에 2~3문장 — 질문형 제목·표·FAQ 라는 껍데기는 갖췄지만 정작 답이 없다.
검색자는 "그래서 내 경우 기준이 뭔데"를 묻는데 문서가 답하지 않으면 2페이지에서 못 올라간다.
1. **각 h2 섹션은 400자 이상**. 단, 표나 체크리스트가 본체인 섹션은 예외.
2. 분량을 늘리려고 같은 말을 다르게 반복하지 마라. 아래 중 최소 2가지를 넣어야 채워진다:
   ① 판단 기준이 되는 수치·범위 (예: "절삭 후 남는 각막이 보통 250~300µm 이상")
   ② 단계별 절차와 각 단계의 소요 기간
   ③ 해당되는 사람 / 해당되지 않는 사람의 구분
   ④ 검사·상담에서 실제로 확인하는 항목
   ⑤ 흔한 오해와 그것이 왜 틀렸는지
3. 수치는 반드시 범위 + 전제와 함께("검사 결과와 각막 상태에 따라 다릅니다").
   단정적 수치·효과 보장은 의료법 위반이자 신뢰도 손실이다.
4. "개인차가 있습니다"로 끝내고 도망가지 마라. 무엇이 개인차를 만드는지(각막 두께·도수·
   나이·기존 질환 등)를 이름으로 적어야 답이 된다.

[E-2. 전환 동선 — 읽는 사람이 스스로 물어보고 싶게]
광고 문구를 넣으라는 뜻이 아니다. **판단에 필요한 정보를 끝까지 주되, 개인 확인이 필요한
지점을 정직하게 남겨두는 것**이 전환을 만든다.
1. 본문 중반(비용·가능 여부·기준을 다루는 섹션) 끝에 "여기서부터는 검사 결과에 따라
   갈린다"는 취지의 문장을 1개 넣어라. 무엇이 갈리는지 구체적으로.
2. 마지막 섹션 직전에 **상담 전 준비 체크리스트**를 넣어라 — 실제로 가져가거나 확인해 둘
   것(현재 안경 도수, 최근 도수 변화, 렌즈 중단 기간, 복용 약, 과거 시술 이력 등).
   이건 영업이 아니라 독자에게 실질적으로 유용하고, 동시에 "물어봐야겠다"를 만든다.
3. 금지: "지금 바로", "서둘러", "선착순", "무료 이벤트" 같은 압박·유인 표현(의료법 위반).
4. 비용은 범위조차 단정하지 말고, **무엇에 따라 달라지는지**를 항목으로 적어라.
...
def _build_blog_user_prompt(
    ...
    # Round 173 (2026-08-23) - 키워드가 질문/롱테일이면 D-3 규칙을 사용자 프롬프트에서
    #   한 번 더 못박는다. 시스템 프롬프트만으로는 모델이 습관적으로 총론형 글을 쓴다.
    _kw = keyword.strip()
    _is_longtail = (
        len(_kw.split()) >= 3
        or _kw.endswith("?")
        or any(m in _kw for m in ("나요", "인가요", "할까요", "차이", "비교", "방법", "기준", "기간", "언제", "얼마나", "몇"))
    )
    if _is_longtail:
        parts.append(
            f"\n[필수 - 질문형 키워드]\n이 키워드는 검색자가 실제로 던지는 질문입니다."
            f" 글 첫 100자 안에 '{_kw}'에 대한 직접적인 답을 한 문장으로 제시하세요."
            " title 은 이 질문을 거의 그대로 담고, 첫 h2 도 이 질문이어야 합니다."
            " 주제를 넓히지 말고 이 질문과 그 후속 질문(기준·기간·비용·주의사항)에만 집중하세요."
        )
